...
class ScriptRunner(QtCore.QObject):
    tick = QtCore.pyqtSignal(float, list)  # (t, relays[8])
    ...
    def run(self):
        ...
        if self._t0 is None:
            ...
            self._t0 = time.perf_counter() - self._pause_t
            self._pause_t = 0.0
        self._paused = False
    ...
    def _on_tick(self):
        ...
        while self._cursor < len(self._events) and t >= float(self._events[self._cursor]["time"]):
            ...
            for i, st in enumerate(rel[:8]):
                self._set_do(i, bool(st))
            last_relays = rel
            self._cursor += 1

        if last_relays is None and self._cursor > 0:
            last_relays = self._events[self._cursor-1].get("relays", [False]*8)
        if last_relays is None:
            last_relays = [False]*8
