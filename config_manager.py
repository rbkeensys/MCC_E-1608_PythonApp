...
class ConfigManager:
    ...
    def load(path: str) -> AppConfig:
        with open(path, "r", encoding="utf-8") as f:
            raw = json.load(f)
        cfg = AppConfig()
        cfg.boardNum = raw.get("boardNum", cfg.boardNum)
        ...
        for i in range(8):
            cfg.analogs[i] = AnalogCfg(
                name=raw.get(f"ai{i}Name", raw.get("analogs", [{}]*8)[i].get("name", f"AI{i}") if isinstance(raw.get("analogs"), list) else f"AI{i}"),
                slope=float(raw.get(f"ai{i}Slope", raw.get("analogs", [{}]*8)[i].get("slope", 1.0) if isinstance(raw.get("analogs"), list) else 1.0)),
                offset=float(raw.get(f"ai{i}Offset", raw.get("analogs", [{}]*8)[i].get("offset", 0.0) if isinstance(raw.get("analogs"), list) else 0.0)),
                cutoffHz=float(raw.get(f"ai{i}FilterCutoffHz", raw.get("analogs", [{}]*8)[i].get("cutoffHz", 0.0) if isinstance(raw.get("analogs"), list) else 0.0)),
                units=(raw.get("analogs", [{}]*8)[i].get("units", "") if isinstance(raw.get("analogs"), list) else ""),
            )
        for i in range(8):
            if isinstance(raw.get("digitalOutputs"), list):
                d = raw["digitalOutputs"][i] if i < len(raw["digitalOutputs"]) else {}
        ...
        for i in range(2):
            if isinstance(raw.get("analogOutputs"), list):
                a = raw["analogOutputs"][i] if i < len(raw["analogOutputs"]) else {}
